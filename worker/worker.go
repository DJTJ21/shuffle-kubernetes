...
var baseimagename = "djrrm01/shuffle"
					...
					if err != nil && !strings.Contains(err.Error(), "Conflict. The container name") {
						...
						if err != nil {
							...
							if len(baseTag) > 1 {
								...
								// Remapping
								ctx := context.Background()
								dockercli.ImageTag(ctx, image, fmt.Sprintf("djrrm01/shuffle:%s", tag))
								dockercli.ImageTag(ctx, image, fmt.Sprintf("registry.hub.docker.com/djrrm01/shuffle:%s", tag))
							}
						}
...
func downloadDockerImageBackend(client *http.Client, imageName string) error {
	...
	if len(baseTag) > 1 {
		...
		// Remapping
		ctx := context.Background()
		dockercli.ImageTag(ctx, imageName, fmt.Sprintf("djrrm01/shuffle:%s", tag))
		dockercli.ImageTag(ctx, imageName, fmt.Sprintf("registry.hub.docker.com/djrrm01/shuffle:%s", tag))
	
		downloadedImages = append(downloadedImages, fmt.Sprintf("djrrm01/shuffle:%s", tag))
		downloadedImages = append(downloadedImages, fmt.Sprintf("registry.hub.docker.com/djrrm01/shuffle:%s", tag))
...
func handleDownloadImage(resp http.ResponseWriter, request *http.Request) {
	...
	for _, img := range images {
		for _, tag := range img.RepoTags {
			...
			possibleNames = append(possibleNames, fmt.Sprintf("djrrm01/shuffle:%s", baseTag))
			possibleNames = append(possibleNames, fmt.Sprintf("registry.hub.docker.com/djrrm01/shuffle:%s", baseTag))
